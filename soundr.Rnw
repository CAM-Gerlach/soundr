...
We save the results of our long in the tooth efforts, and reload it for future uses (so we don't overtax the poor NCEP servers)
...
model.grid.list <- lapply(model.data.list, ModelGrid,
