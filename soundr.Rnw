...
Here, we implement a new technique for diagnosing vertical motion, $\omega$, from simulated GOES sounder temperature data, and verify its effectiveness. Check out Figure~\ref{figure:intro}.
...
First, we need to retrieve the data we'll require.
