...
Thus, we pull down the fields, times and levels needed for analysis with \texttt{rNOMADS}. First, setting our enviroment up and selecting the fields we want:

<<setupdata, cache=TRUE>>=
library(rNOMADS, quietly = TRUE)
...
And re-load it from our serialized file:
