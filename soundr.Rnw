...
<<matrixdata, cache=TRUE, results="hide">>=
# Setup
library(parallel)
...
Now, we organize what we've got by pressure level, and then by time, and finally by variable. We'll use some fancy tricks with the apply family of "meta-functions" to avoid for loops.
...
So, our data structure now sorta-kinda-eh makes sense for raw processing, but what if we want to visualize it in \texttt{ggplot}, or do other nice R things with it? Well, for that, we'll need data frames! Not exactly designed for n-dimensional data, but we can hack it...
